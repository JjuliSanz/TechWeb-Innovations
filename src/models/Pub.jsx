...
import pubScene from "../assets/3d/cyberpunk_bar.glb";

export function Pub ({ isRotating, setIsRotating, setCurrentStage, currentFocusPoint, ...props }) {
  const { nodes, materials } = useGLTF(pubScene);
  ...
  const handlePointerMove = (event) => {
    ...
    if (isRotating) {
      ...
      pubRef.current.rotation.z += delta * 0.01 * Math.PI;
  ...
  const handleKeyDown = (e) => {
    if (e.key === "ArrowLeft") {
      ...
      pubRef.current.rotation.z += 0.01 * Math.PI;
  ...
  useFrame(() => {
    if (!isRotating) {
      ...
      pubRef.current.rotation.z += rotationSpeed.current;
    } else {
      const rotation = pubRef.current.rotation.z;
